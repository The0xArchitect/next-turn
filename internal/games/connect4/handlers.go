// internal/games/connect4/handlers.go
package connect4
...
func NewHandlers(database *db.DB) *Handlers {
	return &Handlers{
		DB:     database,
		Engine: core.Get("connect4").(*Engine),
	}
}
...
func (h *Handlers) onMove(bot *tgbotapi.BotAPI, cb *tgbotapi.CallbackQuery, parts []string) {
	game, imid := h.getGame(bot, cb, parts)
	if game == nil {
		return
	}
	...
	column, err := strconv.Atoi(parts[3])
	if err != nil {
		answerCallback(bot, cb.ID, "Invalid move")
		return
	}
	...
	if !h.Engine.IsValidMove(game, column) {
		answerCallback(bot, cb.ID, "Column is full!")
		return
	}

	newBoard := h.Engine.ApplyMove(game.Board, column, game.CurrentTurn)
	nextTurn := h.Engine.NextTurn(game.CurrentTurn)

	ctx := context.Background()
	updated, err := h.DB.UpdateBoard(ctx, game.ID, newBoard, nextTurn)
	if err != nil || updated == nil {
	...
	result := h.Engine.CheckResult(updated.Board)
	if result != nil {
		var gameResult string
		if *result == "draw" {
			gameResult = "draw"
		} else {
			winnerIsCreator := (updated.XPlayer == "creator" && *result == "X") ||
				(updated.XPlayer == "opponent" && *result == "O")
			if winnerIsCreator {
				gameResult = "creator"
			} else {
				gameResult = "opponent"
