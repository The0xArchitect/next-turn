// internal/games/tictactoe/handlers.go
package tictactoe
...
func NewHandlers(database *db.DB) *Handlers {
	return &Handlers{
		DB:     database,
		Engine: core.Get("tictactoe").(*Engine),
	}
}

// HandleCallback routes ttt:* callbacks.
...
func (h *Handlers) onMove(bot *tgbotapi.BotAPI, cb *tgbotapi.CallbackQuery, parts []string) {
	...
	if !h.Engine.IsValidMove(game, position) {
		answerCallback(bot, cb.ID, "Invalid move")
		return
	}

	newBoard := h.Engine.ApplyMove(game.Board, position, game.CurrentTurn)
