// internal/games/tictactoe/ui.go
package tictactoe
...
func LobbyWaiting(gameID string) tgbotapi.InlineKeyboardMarkup {
	return tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("Join", fmt.Sprintf("ttt:join:%s", gameID)),
			tgbotapi.NewInlineKeyboardButtonData("Quit", fmt.Sprintf("ttt:quit:%s", gameID)),
		),
	)
}
...
func LobbyReady(gameID string) tgbotapi.InlineKeyboardMarkup {
	return tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("Start", fmt.Sprintf("ttt:start:%s", gameID)),
			tgbotapi.NewInlineKeyboardButtonData("Quit", fmt.Sprintf("ttt:quit:%s", gameID)),
		),
	)
}
...
func Board(gameID, boardState string, frozen bool) tgbotapi.InlineKeyboardMarkup {
	...
	for row := 0; row < 3; row++ {
		...
		for col := 0; col < 3; col++ {
			...
			cbData := "noop"
			if !frozen && cell == '-' {
				cbData = fmt.Sprintf("ttt:move:%s:%d", gameID, pos)
			}
...
func Finished(game *models.Game) tgbotapi.InlineKeyboardMarkup {
	...
	kb.InlineKeyboard = append(kb.InlineKeyboard, tgbotapi.NewInlineKeyboardRow(
		tgbotapi.NewInlineKeyboardButtonData("🔄 Play Again", fmt.Sprintf("ttt:rematch:%s", game.ID)),
	))
	return kb
}
