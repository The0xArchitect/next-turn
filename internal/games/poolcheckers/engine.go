// internal/games/poolcheckers/engine.go
...
// FLYING KING: Kings can move/capture across multiple diagonal squares
...
func (e *Engine) GameType() string     { return "poolcheckers" }
func (e *Engine) DisplayName() string  { return "🎱 Pool Checkers" }
func (e *Engine) Description() string  { return "Checkers with flying kings!" }
func (e *Engine) ThumbnailURL() string { return "pool-checkers.jpg" }
...
func (e *Engine) ApplyCheckerMove(board string, from, to int, isWhite bool) MoveResult {
	...
	// Multi-jump: check for more jumps (even if just promoted - flying kings can continue)
	mustContinue := false
	continueFrom := -1
	if jump != nil {
...
func getValidMoves(board string, pos int, isWhite bool) []int {
	...
	if isKing {
		...
		// Regular man: moves one square diagonally forward
		forwardDir := 1
		if isWhite {
			forwardDir = -1
		}
		for _, dCol := range []int{-1, 1} {
			newRow := row + forwardDir
			newCol := col + dCol
	...
	if isKing {
		...
		for _, dRow := range []int{-1, 1} {
			for _, dCol := range []int{-1, 1} {
				...
				for scanRow >= 0 && scanRow < size && scanCol >= 0 && scanCol < size {
					scanPos := scanRow*size + scanCol
					cell := board[scanPos]

					if cell == '-' {
						if capturedPos != -1 {
							// Found landing spot after enemy piece
							jumps = append(jumps, JumpMove{
								From:     pos,
								To:       scanPos,
								Captured: capturedPos,
							})
						}
						// Continue scanning for more landing spots
					} else if isEnemyPiece(cell, isWhite) && capturedPos == -1 {
						// Found enemy piece to potentially capture
						capturedPos = scanPos
					} else {
						// Blocked by own piece or second enemy piece
						break
					}
					scanRow += dRow
					scanCol += dCol
		...
		// Regular man: standard jump (can jump forward and backward for captures)
		for _, dRow := range []int{-1, 1} {
			for _, dCol := range []int{-1, 1} {
				midRow := row + dRow
				midCol := col + dCol
...
// --- GameEngine interface (unused stubs for pool checkers) ---
...
func (e *Engine) ApplyMove(board string, position int, symbol string) string {
	return board // Not used for pool checkers
}
